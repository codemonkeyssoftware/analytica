...
class Analytica_PublicApi
{
	...
	public function registerClass( $class )
	{
		...
		if(!$rClass->isSubclassOf(new ReflectionClass("Analytica_Apiable")))
		{
			throw new Exception("To publish its public methods in the API, the class '$class' must be a subclass of 'Analytica_Apiable'.");
		}

		Analytica::log("List of the public methods for the class $class");
		...
		foreach($rMethods as $method)
		{
			...
			)
			{
				...
				Analytica::log("- $name is public ".$this->getStrListParameters($class, $name));
			}
		}
	}
	...
	public function __call($methodName, $parameters )
	{
		...
		$className = Analytica::prefixClass(self::$classCalled);
		...
		Analytica::log("Calling ".self::$classCalled.".$methodName [$args]");
		...
		{
			Analytica::log("Error during API call...". $e->getMessage());
			exit;
		}
