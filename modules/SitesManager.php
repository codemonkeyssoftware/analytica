...
class Analytica_SitesManager extends Analytica_APIable
{
	...
	static private function insertSiteUrls($idSite, $aUrls)
	{
		...
		foreach($aUrls as $url)
		{
			$db->insert(Analytica::prefixTable("site_url"), array(
									'idsite' => $idSite,
									'url' => $url
									)
								);
	...
	{
		$db = Zend_Registry::get('db');
		$db->query("DELETE FROM ".Analytica::prefixTable("site_url") ." WHERE idsite = ?", $idsite);
	}
	...
	{
		$db = Zend_Registry::get('db');
		$urls = $db->fetchCol("SELECT url FROM ".Analytica::prefixTable("site_url"). " WHERE idsite = ?", $idsite);
		return $urls;
	}
	...
	{
		$db = Zend_Registry::get('db');
		$idSites = $db->fetchCol("SELECT idsite FROM ".Analytica::prefixTable('site'));
		return $idSites;
	}
