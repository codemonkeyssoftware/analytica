...
class Analytica_UsersManager extends Analytica_APIable
{
    ...
    static public function getUser($login)
    {
        ...
        $users = $db->fetchCol("SELECT *
                FROM " . Analytica::prefixTable("user")
                . " WHERE login = ?", $login);
        return $users;
    }
	...
	static public function addUser( $userLogin, $password, $alias, $email )
	{
		...
		$db->insert( Analytica::prefixTable("user"), array(
									'login' => $userLogin,
	...
	static public function deleteUser( $userLogin )
	{
		...
		$db = Zend_Registry::get('db');
		$db->query("DELETE FROM ".Analytica::prefixTable("user")." WHERE login = ?", $userLogin);
    ...
    static public function setUserRole($access, $userLogin, $idSites = null)
	{
		...
		if(is_null($idSites))
		{
			$idSites = Analytica_SitesManager::getSitesIdWithAdminAccess();
		}
		...
		{		
			if (!Analytica_SitesManager::siteExists($idsite))
			{
		...
		foreach($idSites as $idsite)
		{
			$db->query(	"DELETE FROM ".Analytica::prefixTable("role").
		...
		if($access != "noaccess")
		{
			foreach($idSites as $idsite)
			{
				$db->insert(	Analytica::prefixTable("role"),
								array(	"idsite" => $idsite, 
										"login" => $userLogin,
										"role" => $access)
						);
